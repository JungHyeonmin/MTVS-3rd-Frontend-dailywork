 ...
 * - 배열을 정렬 기준으로 정렬한다.
 *
 * - 오름차순 정렬이 기본이면 정렬 후 정렬 순서를 유지한다.
 * - 배열은 기본적으로 문자열 정럴이 되므로 일부 올바르지 않게 정렬 되는 모습을 확인할 수 있다.
 * - 다른 정렬 기준을 사용하려면 매개변수로 compare 함수를 전달해야 한다.
 *
 * Array.prototype.forEach
 * - for 를 대체할 수 있는 고차함수이다.
 *
 * Array.prototype.map
 * - 배열 요소 전체를 대상으로 콜백 함수 호출 후 반환 값들로 구성 된 새로운 배열을 반환한다.
 *
 * Array.prototype.filter
 * - 배열 요소 전체를 대상으로 콜백 함수 호출 후 반환 값이 true 인 요소로만 구성된 새로운 배열 반환한다.
 *
 * Array.prototype.reduce
 * - 배열을 순회하며 각 요소에 대하여 이전 콜백함수 실행 반환값을 전달하여 콜백함수를 실행하고 그 결과를 반환한다.
 *
 * Array.prototype.some
 * - 배열 내 일부 요소가 콜백 함수의 테스트를 통과하는지 확인하여 그 결과를 boolean 으로 반환한다.
 *
 * Array.prototype.every
 * - 배열 내 모든 요소가 콜백함수의 테스트를 통과하는지 확인하여 그 결과를 boolean 으로 반환한다.
 *
 * Array.prototype.find, Array.prototype.findIndex
 * - find : 배열을 순회하먀 각 요소에 대하여 인자로 주어진 콜백함수를 실행하여 그 결과가 참인 첫번째 요소를 반환
 *      - 참인 요소가 존재하지 않는다면 첫번째 요소를 반환
 * - findIndex : 배열을 순회하며 각 요소에 대하여 인자로 주어진 콜백함수를 실행하여 그 결과가 참인 첫번째 요소의 인덱스를 반환
 *      - 참인 요소가 존재하지 않는다면 '-1'을 반환
 */
...
for (let i = 0; i < 10; i++) {
    ...
    numbers2[i] = Math.floor(Math.random() * 100) + 1;
}
...
// 자신이 원하는 방식으로 정렬하는 compare 함수
// 숫자 오름차순 정렬
function compare(a, b) {
    if (a > b) return 1;  // 양수 : 오름차순
    if (a == b) return 0; // 0 : 변경없음
    if (a < b) return -1; // 음수 : 내림차순
}

numbers2.sort(compare);
console.log('매개변수로 compare 함수 전달하여 숫자 오름차순 정렬');
console.log(`compare 정렬 전 numbers2 : ${numbers2}`);
...
console.log('numbers2 숫자 내림차순 정렬');
console.log(numbers2);
