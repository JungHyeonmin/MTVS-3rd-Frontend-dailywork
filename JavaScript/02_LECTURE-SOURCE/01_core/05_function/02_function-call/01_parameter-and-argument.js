 ...
 * - 인수를 전달하지 않았을 경우, undefined 를 전달하였을 경우 ES6에서
 *   도입된 매개변수 기본값을 사용할 수 있다.
 * - 매개변수의 최대 개수에 대해 명시적인 제한은 없다.
 *      - 하지만 이상적인 함수는 한가지 일만 해야 하며, 가급적 작게 만들어야 하므로
...
function goodbye(name) {
    if (arguments.length !== 1 || typeof name !== 'string' || name.length === 0) {
        throw new TypeError('인자는 1개여야합니다.');
    }
    return `${name} 안녕~~~`;
}

var result = goodbye('needMoreCash'); // 문자열 1개가 아니면 에러
console.log(result);