 ...
 * - 함수의 변하지 않는 공통 로직은 미리 정의해두고 경우에 따라 변경되는 로직은 추상화해서
 *   함수 외부에서 내부로 전달하는 방식이다.
 * - 콜백 함수는 함수형 프로그래밍 패러다임뿐만 아니라 비동기 처리 (이벤트, 타이머, ajax)에 활용되는
 *   중요한 패턴이다. 또한 배열 고차 함수(map)에서도 사용 된다.
 */
...
// 콜백 함수 : 함수의 매개변수로 함수를 받아서 인자를 주고 함수 내에서 함수를 실행
console.log(apply(increase, 5));
console.log(apply(decrease, 5));

console.log(apply(function (value) {return value ** 2}, 5));

console.log([3, 4, 5, 1, 2].sort(function (left, right) {
    return right - left;
}));